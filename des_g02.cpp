...
u64 *converted = new u64[21474836];
int convertedSize = 0;
u64 keys[16] = {0};
...
{
    // array of characters
        unsigned char *array1 = new unsigned char[21474836];
        int array_length = 0;
        // filling the array
        char ch;
        ifstream file;
        file.open(inputFile, ios_base::binary);
        int c = 0;
        while (!file.eof())
        {
            file >> noskipws >> ch;
            array1[c++] = (unsigned)ch;
        }
        array_length = c;
...
void outputPlainText(u64 cypher, string outputFile)
{
    int value = 0;
        ...
        ofstream myFile(outputFile, ios_base::app | ios_base::binary);
        char c;
        for (int i = 0; i < 8; i++)
        {
